...
if ($responseCode == 200) {
  try {
    validateJSONObject($decodedJSON, ['residence_id'], true);
  } catch(Exception $e) {
    $responseCode = 400;
...
if ($responseCode == 200) {
    try {
        ...
        $query = "UPDATE residence SET deleted = 1
            WHERE id = ?";
        $paramTypes = 'i';
        $params = [$residenceID];
        $message = "Residence record deleted.";
        ...
        $responseCode = 400;
        array_push($errors, buildError('Error deleting residence.', $e->getMessage()));
    } finally {
      /* close connection */
